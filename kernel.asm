...
80103632:	50                   	push   %eax
  ...
  *(uint*)sp = (uint)trapret;
  ...
  sp -= sizeof *p->context;
  p->context = (struct context*)sp;
80103633:	89 43 20             	mov    %eax,0x20(%ebx)
  memset(p->context, 0, sizeof *p->context);
...
801037f3:	ff 73 1c             	pushl  0x1c(%ebx)
...
8010380b:	66 89 50 3c          	mov    %dx,0x3c(%eax)
  p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
8010380f:	8b 43 1c             	mov    0x1c(%ebx),%eax
...
8010381d:	66 89 50 28          	mov    %dx,0x28(%eax)
  p->tf->ss = p->tf->ds;
80103821:	8b 43 1c             	mov    0x1c(%ebx),%eax
...
80103839:	c7 40 44 00 10 00 00 	movl   $0x1000,0x44(%eax)
  p->tf->eip = 0;  // beginning of initcode.S
80103840:	8b 43 1c             	mov    0x1c(%ebx),%eax
80103843:	c7 40 38 00 00 00 00 	movl   $0x0,0x38(%eax)

  safestrcpy(p->name, "initcode", sizeof(p->name));
8010384a:	8d 43 70             	lea    0x70(%ebx),%eax
...
80103866:	89 43 6c             	mov    %eax,0x6c(%ebx)
...
80103970:	89 01                	mov    %eax,(%ecx)
  np->parent = curproc;
80103972:	89 59 18             	mov    %ebx,0x18(%ecx)
  *np->tf = *curproc->tf;
80103975:	89 c8                	mov    %ecx,%eax
80103977:	8b 79 1c             	mov    0x1c(%ecx),%edi
8010397a:	8b 73 1c             	mov    0x1c(%ebx),%esi
...
80103984:	31 f6                	xor    %esi,%esi
  ...
  *np->tf = *curproc->tf;
  ...
  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;
80103986:	8b 40 1c             	mov    0x1c(%eax),%eax
80103989:	c7 40 1c 00 00 00 00 	movl   $0x0,0x1c(%eax)

  for(i = 0; i < NOFILE; i++)
    if(curproc->ofile[i])
80103990:	8b 44 b3 2c          	mov    0x2c(%ebx,%esi,4),%eax
...
801039a7:	89 44 b2 2c          	mov    %eax,0x2c(%edx,%esi,4)
  *np->tf = *curproc->tf;
...
801039b6:	ff 73 6c             	pushl  0x6c(%ebx)

  safestrcpy(np->name, curproc->name, sizeof(curproc->name));
801039b9:	83 c3 70             	add    $0x70,%ebx
  np->tf->eax = 0;
...
801039c4:	83 c4 0c             	add    $0xc,%esp
  ...
  for(i = 0; i < NOFILE; i++)
    if(curproc->ofile[i])
      np->ofile[i] = filedup(curproc->ofile[i]);
  np->cwd = idup(curproc->cwd);
801039c7:	89 47 6c             	mov    %eax,0x6c(%edi)

  safestrcpy(np->name, curproc->name, sizeof(curproc->name));
801039ca:	8d 47 70             	lea    0x70(%edi),%eax
...
80103b3e:	83 c3 20             	add    $0x20,%ebx
    panic("sched locks");
  if(p->state == RUNNING)
    panic("sched running");
  if(readeflags()&FL_IF)
...
80103bd4:	8d 5e 2c             	lea    0x2c(%esi),%ebx
80103bd7:	8d 7e 6c             	lea    0x6c(%esi),%edi
...
80103c27:	8b 56 18             	mov    0x18(%esi),%edx
...
80103c48:	3b 50 24             	cmp    0x24(%eax),%edx
...
80103c79:	74 33                	je     80103cae <exit+0xfe>
    if(p->parent == curproc){
80103c7b:	39 72 18             	cmp    %esi,0x18(%edx)
...
80103c80:	83 7a 0c 05          	cmpl   $0x5,0xc(%edx)
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->parent == curproc){
      p->parent = initproc;
80103c84:	89 4a 18             	mov    %ecx,0x18(%edx)
      if(p->state == ZOMBIE)
...
80103ca0:	3b 48 24             	cmp    0x24(%eax),%ecx
...
80103cb1:	c7 46 0c 05 00 00 00 	movl   $0x5,0xc(%esi)
 ...
 curproc->exit_status = status;
80103cb8:	89 46 14             	mov    %eax,0x14(%esi)
...
80103cc3:	68 bd 74 10 80       	push   $0x801074bd
...
80103cd0:	68 b0 74 10 80       	push   $0x801074b0
...
80103d78:	e8 43 06 00 00       	call   801043c0 <release>
  ...
  // Go to sleep.
  p->chan = chan;
80103d7d:	89 7b 24             	mov    %edi,0x24(%ebx)
  p->state = SLEEPING;
...
80103d87:	e8 64 fd ff ff       	call   80103af0 <sched>
  ...
  // Tidy up.
  p->chan = 0;
80103d8c:	c7 43 24 00 00 00 00 	movl   $0x0,0x24(%ebx)
...
80103db1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  ...
  // Go to sleep.
  p->chan = chan;
80103db8:	89 7b 24             	mov    %edi,0x24(%ebx)
  p->state = SLEEPING;
...
80103e31:	74 1d                	je     80103e50 <wait+0x60>
      if(p->parent != curproc)
80103e33:	39 73 18             	cmp    %esi,0x18(%ebx)
...
80103e54:	8b 46 28             	mov    0x28(%esi),%eax
...
80103e8c:	c7 43 10 00 00 00 00 	movl   $0x0,0x10(%ebx)
        p->parent = 0;
80103e93:	c7 43 18 00 00 00 00 	movl   $0x0,0x18(%ebx)
        p->name[0] = 0;
80103e9a:	c6 43 70 00          	movb   $0x0,0x70(%ebx)
        p->killed = 0;
80103e9e:	c7 43 28 00 00 00 00 	movl   $0x0,0x28(%ebx)
        p->state = UNUSED;
...
80103f10:	3b 58 24             	cmp    0x24(%eax),%ebx
...
80103f6f:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->pid == pid){
      p->killed = 1;
80103f73:	c7 40 28 01 00 00 00 	movl   $0x1,0x28(%eax)
      // Wake process from sleep if necessary.
      if(p->state == SLEEPING)
...
80103fd9:	bb c4 2d 11 80       	mov    $0x80112dc4,%ebx
...
80103ff8:	83 eb 80             	sub    $0xffffff80,%ebx
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80103ffb:	81 fb c4 4d 11 80    	cmp    $0x80114dc4,%ebx
80104001:	0f 84 81 00 00 00    	je     80104088 <procdump+0xb8>
    if(p->state == UNUSED)
80104007:	8b 43 9c             	mov    -0x64(%ebx),%eax
...
80104815:	e8 66 ef ff ff       	call   80103780 <myproc>
  ...
  num = curproc->tf->eax;
8010481a:	8b 70 1c             	mov    0x1c(%eax),%esi
...
80104848:	50                   	push   %eax
            curproc->pid, curproc->name, num);
80104849:	8d 43 70             	lea    0x70(%ebx),%eax
...
80104855:	e8 06 be ff ff       	call   80100660 <cprintf>
            curproc->pid, curproc->name, num);
    curproc->tf->eax = -1;
8010485a:	8b 43 1c             	mov    0x1c(%ebx),%eax
...
80104a3c:	8b 44 90 2c          	mov    0x2c(%eax,%edx,4),%eax
...
80104aa0:	8b 54 98 2c          	mov    0x2c(%eax,%ebx,4),%edx
...
80104abc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  ...
  for(fd = 0; fd < NOFILE; fd++){
    if(curproc->ofile[fd] == 0){
      curproc->ofile[fd] = f;
80104ac0:	89 74 98 2c          	mov    %esi,0x2c(%eax,%ebx,4)
...
80104bdd:	83 ec 0c             	sub    $0xc,%esp
  ...
  if(argfd(0, &fd, &f) < 0)
    return -1;
  myproc()->ofile[fd] = 0;
80104be0:	c7 44 90 2c 00 00 00 	movl   $0x0,0x2c(%eax,%edx,4)
80104be7:	00 
  fileclose(f);
...
80105039:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  ...
  for(fd = 0; fd < NOFILE; fd++){
    if(curproc->ofile[fd] == 0){
80105040:	8b 54 98 2c          	mov    0x2c(%eax,%ebx,4),%edx
...
801050b0:	83 ec 0c             	sub    $0xc,%esp
  ...
  for(fd = 0; fd < NOFILE; fd++){
    if(curproc->ofile[fd] == 0){
      curproc->ofile[fd] = f;
801050b3:	89 7c 98 2c          	mov    %edi,0x2c(%eax,%ebx,4)
      fileclose(f);
    iunlockput(ip);
    end_op();
    return -1;
...
801053ce:	66 90                	xchg   %ax,%ax
  ...
  for(fd = 0; fd < NOFILE; fd++){
    if(curproc->ofile[fd] == 0){
801053d0:	8b 74 98 2c          	mov    0x2c(%eax,%ebx,4),%esi
...
8010540b:	89 7c b0 0c          	mov    %edi,0xc(%eax,%esi,4)
  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
    return -1;
  if(pipealloc(&rf, &wf) < 0)
    return -1;
...
80105435:	c7 44 b0 0c 00 00 00 	movl   $0x0,0xc(%eax,%esi,4)
8010543c:	00 
...
8010543f:	90                   	nop
  ...
  for(fd = 0; fd < NOFILE; fd++){
    if(curproc->ofile[fd] == 0){
      curproc->ofile[fd] = f;
80105440:	89 7c 90 2c          	mov    %edi,0x2c(%eax,%edx,4)
      myproc()->ofile[fd0] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
...
80105473:	83 ec 1c             	sub    $0x1c,%esp
  int status;
  argptr(0, (char **) &status, 4);
80105476:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105479:	6a 04                	push   $0x4
8010547b:	50                   	push   %eax
8010547c:	6a 00                	push   $0x0
8010547e:	e8 ed f2 ff ff       	call   80104770 <argptr>
  exit(status);
80105483:	58                   	pop    %eax
80105484:	ff 75 f4             	pushl  -0xc(%ebp)
80105487:	e8 24 e7 ff ff       	call   80103bb0 <exit>
...
801055a6:	8b 40 28             	mov    0x28(%eax),%eax
...
8010578d:	53                   	push   %ebx
            "eip 0x%x addr 0x%x--kill proc\n",
            myproc()->pid, myproc()->name, tf->trapno,
8010578e:	83 c2 70             	add    $0x70,%edx
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpuid(), tf->eip, rcr2());
      panic("trap");
    }
...
801057a7:	c7 40 28 01 00 00 00 	movl   $0x1,0x28(%eax)
...
801057be:	8b 50 28             	mov    0x28(%eax),%edx
...
801057e7:	8b 40 28             	mov    0x28(%eax),%eax
...
80105918:	e8 83 e9 ff ff       	call   801042a0 <acquire>
      ticks++;
      wakeup(&ticks);
8010591d:	c7 04 24 a0 55 11 80 	movl   $0x801155a0,(%esp)
  ...
  case T_IRQ0 + IRQ_TIMER:
    if(cpuid() == 0){
      acquire(&tickslock);
      ticks++;
80105924:	83 05 a0 55 11 80 01 	addl   $0x1,0x801155a0
      wakeup(&ticks);
...
80106820:	a1 a4 55 11 80       	mov    0x801155a4,%eax
